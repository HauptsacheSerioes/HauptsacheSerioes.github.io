...
class Main extends React.Component {
  ...
  _replaceSVGs = () => {
    ...
    for (let i = 0; i < svgImages.length; i++) {
      const id = svgImages[i].id;
      const classes = svgImages[i].className;
      const src = svgImages[i].dataset.src;

      $.get(src)
      .then(res => {
        const svg = $(res).find('svg')[0];
  ...
  _handleMouseMove = (evt) => {
    if (!this._onMobile) {
      this.refs.header.handleMouseMove(evt);
    }
  }
